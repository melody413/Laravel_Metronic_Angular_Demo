...
export class FaqListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "faqs/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "faqs/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "faqs/table", {
  ...
  delete(id: number){
    if(confirm("Do you really delete this data?")){
      this.http.get<any>(environment.apiUrl+ "faqs/delete/" + id)
        .subscribe((response)=>{
          this.ngOnInit();
        })
    }
