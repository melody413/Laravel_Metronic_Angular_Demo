...
export class CityListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "city/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "city/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "city/table", {
  ...
  delete(id: number){
    this.confirmationService.confirm({
        ...
        accept: () => {
          this.http.get<any>(environment.apiUrl+ "city/delete/" + id)
          .subscribe((response)=>{
