...
export class RoleListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "role/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "role/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "role/table", {
  ...
  delete(id: number){
    this.confirmationService.confirm({
        ...
        accept: () => {
          this.http.get<any>(environment.apiUrl+ "role/delete/" + id)
          .subscribe((response)=>{
