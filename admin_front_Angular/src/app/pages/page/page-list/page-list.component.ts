...
export class PageListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "pages/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "pages/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "pages/table", {
  ...
  //delete the data
  delete(id: number){
    if(confirm("Do you really delete data?")){
      this.http.get<any>(environment.apiUrl+ "pages/delete/" + id)
        .subscribe((response)=>{
          this.ngOnInit();
        })
    }
