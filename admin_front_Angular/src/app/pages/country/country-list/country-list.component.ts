...
export class CountryListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "country/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "country/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "country/table", {
  ...
  delete(id: number){
    if(confirm("Do you really delete the data?")){
      this.http.get<any>(environment.apiUrl+ "country/delete/" + id)
        .subscribe((response)=>{
          this.ngOnInit();
        })
    }
