...
export class AreaListComponent implements OnInit{
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "area/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "area/table", {
  ...
  //delete the data
  delete(id: number){
    this.http.get<any>(environment.apiUrl+ "area/delete/" + id)
      .subscribe((response)=>{
        this.ngOnInit();
      })
  }
