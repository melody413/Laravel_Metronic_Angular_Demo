...
export class QuestionAnswerListComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.http.get<any>(environment.apiUrl + "qanswer/list").
      subscribe((response) => {        
  ...
  search(){
    if(this.search_index == ""){
      ...
      this.http.post<any>(environment.apiUrl + "qanswer/table", {"search_index": this.search_index.toString()})   
          .subscribe((response)=>{
  ...
  pageChange(){
    if(this.paginator.pageIndex == 0 && this.paginator.pageSize == 10){
      ...
      this.http.post<any>(environment.apiUrl + "qanswer/table", {
