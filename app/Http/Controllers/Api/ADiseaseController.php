...
class ADiseaseController extends BaseController
{
    ...
    public function index(Request $request)
    {
        $query = Disease::query();
        $data = dataTable()
            ->of($query->orderByRaw('created_at DESC') )
    ...
    public function create()
    {
        ...
        return response(['diseases' => $diseases, "body_parts"=>$body_parts], 200);
    }
        ...
        if($request->has("search_index")){
            ...
            $query = DB::table(function ($query) use ($searchIndex) {
                $query->select(
                        ...
                        'disease_trans_en.NAME AS enname'
                    )
                    ->from('diseases')
                    ->join('disease_trans', 'diseases.id', '=', 'disease_trans.disease_id')
                    ->join('disease_trans AS disease_trans_en', 'diseases.id', '=', 'disease_trans_en.disease_id')
                    ->where('disease_trans.locale', 'ar')
                    ->where('disease_trans_en.locale', 'en');
            }, 'result1')
            ->select(
                'result1.*',
                'result1.enname AS enname'
            )
            ->where('result1.arname', 'LIKE', "%$searchIndex%")
            ->orWhere('result1.enname', 'LIKE', "%$searchIndex%"); 
