...
@section('content')
    <section class="inner_container unit insurance_company_unit">
        <div class="container">
            {{-- @if ($specialty->first() && isset($row->city->name) && isset($row->area_id))
                @include('frontend.partials.breadcrumb', [
                    'breadcrumb' => [
                        ...
                        $row->city->name => route('frontend.hospital.index') . '?city=' . $row->city_id,
                        $row->area->name => route('frontend.hospital.index') . '?area=' . $row->area_id. '&city=' . $row->city_id,
                        \App\Models\Specialty::find( $specialty->first() )->name => route('frontend.hospital.index'). '?speciality=' .$specialty->first(). '&city=' . $row->city->id. '&area=' . $row->area->id,
                    ]
                ])
            ...
            @if($row->hospital_types->first())
                <h5>{!! '<i class="fas fa-hospital"></i> ' . $row->hospital_types->first()->name !!}</h5>
            @endif

            @unit_block(['row' => $row ,'imagePath' => 'hospitals/', 'withAddress' => true, 'specialty' => $specialty])
                ...
                @slot('titlePlus')
                    ...
                    <span id="specialty">
                        ...
                        @foreach ($specialty as $specialty_row)
                                <a class="specialty-row" href="{{ route('frontend.hospital.index') . '?speciality='. $specialty_row }}">
                    ...
                    <div id="lightgallery" class="unit_images flexer flexer_ac_space_between flexer_ai_stretch flexer_wrap">
                        @if(!empty($row->image_gallery))
                            @foreach (json_decode($row->image_gallery) as $ga)
                                <a href="/uploads/hospitals/{{ $ga }}">
                                    <img src="/uploads/hospitals/{{ $ga }}" />
                                </a>
                            @endforeach
                        @endif
                    </div>
                ...
                @slot('topping')
                        ...
                        <div class="content">
                            <div class="topping_tabs">
                                <ul class="nav nav-tabs" id="topping_tabs_ul" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" id="hospitals-tab" data-toggle="tab" href="#hospitals" role="tab" aria-controls="profile" aria-selected="false">{{ trans('general.insurances_companies') }}</a>
                                    </li>
                                </ul>
                    ...
                    @if($row->tags_ar)
                        ...
                        @foreach ( $tags_arr as $tg)
                            <a href="{{ route('frontend.hospital.index') . '?speciality=' . $specialty[0] . '&tag=' . $tg}}">
                    ...
                    @else
                        @foreach ($tags as $tag_s)
                            @foreach ($tag_s->specialties()->whereIn('specialty_id', $specialty)->get() as $tg)
                                <a href="{{ route('frontend.hospital.index') . '?speciality=' . $specialty[0] . '&tag=' . $tg->pivot->tag_id}}">
                                    <span style="color: #fff" class="badge badge-primary">
                    ...
                    <hr>
                    <style>
                        span.specialty-hospital:last-child {display: none;}
                    </style>
