        ...
        <div class="col-sm-8">
            @foreach($permission['permissions'] as $row)
                @if( ! isset($row['hidden']) || $row['hidden'] != true )
                    {!! Form::checkbox( 'roles_premissions[]' , 'admin.' . $permission['key'] . '.' . $row['key'] , @in_array('admin.' . $permission['key'] . '.' . $row['key'], $permissions)?true:false , ['id' => 'prm_' . $permission['group'] . '.' . $permission['key'] . '.' . $row['key'] ]) !!}  <label for="prm_{{ $permission['group'] . '.' . $permission['key'] . '.' . $row['key'] }}">{!! $row['label'] !!}</label>
                @endif
            @endforeach
        </div>
    </div>
