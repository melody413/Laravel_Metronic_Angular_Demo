...
<div class="col-sm-12">
	<br>
	@include('admin.partial._form_image', ['input' => 'image', 'label' => 'Logo', 'path' => 'hospitals'])
   ...
   <br>
	...
	<div class="col-sm-10 center center-align">
		<div class="demo-checkbox">
			<div class="row">
			@foreach(dataForm()->getHospitalTypes() as $row)

				{!! Form::checkbox('hospital_types[]', $row->id, isset($hospitalTypeIds) && in_array($row->id, $hospitalTypeIds), array('id'=> 'hospital_types_'.$row->id,'class' => 'filled-in chk-col-brown')) !!}
				<label for="hospital_types_{{ $row->id }}">{{ $row->name }}</label>
			@endforeach
			</div>
		</div>
	</div>
